...
<div class="max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow-md mt-6">
    ...
    <form action="{{ route('organisasi.update', optional($profile)->id ?? 0) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" class="input-field" value="{{ old('name', $profile->name ?? '') }}">
                </div>
                <div>
                    <label class="block mb-1 font-medium">Email</label>
                    <input type="email" name="email" class="input-field" value="{{ old('email', $profile->email ?? '') }}">
                </div>
                <div>
                    <label class="block mb-1 font-medium">No. Telepon</label>
                    <input type="text" name="phone" class="input-field" value="{{ old('phone', $profile->phone ?? '') }}">
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="nama_organisasi" class="input-field" value="{{ old('nama_organisasi', $profile->nama_organisasi ?? '') }}">
                </div>
                <div>
                    <label class="block mb-1 font-medium">Tipe Organisasi</label>
                    <input type="text" name="tipe_organisasi" class="input-field" value="{{ old('tipe_organisasi', $profile->tipe_organisasi ?? '') }}">
                </div>
                <div>
                ...
                <div>
                    ...
                    <textarea name="description" class="input-field">{{ old('description', $profile->description ?? '') }}</textarea>
                </div>
                <div>
